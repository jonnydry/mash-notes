...
test.describe('Permanent welcome branding', () => {
	test('shows Spoon on the pinned card, library row, and large editor', async ({ page }) => {
		...
		await expect(card.getByAltText('Spoon, the Mash mascot')).toBeVisible();
		...
		await expect(row.getByAltText('Spoon, the Mash mascot')).toBeVisible();
		await row.dblclick();

		const stage = page.getByRole('region', { name: 'Note editor stage' });
		await expect(stage.getByAltText('Spoon, the Mash mascot')).toBeVisible();
	});
});
